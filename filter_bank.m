...
x_v_H_tilde = zeros(size(x_h_H_down));
x_v_G_tilde = zeros(size(x_h_G_down));
...
x_v_H_down = zeros(ceil(size(x_h_H_down,1)/2),size(x_h_H_down,2));
x_v_G_down = zeros(ceil(size(x_h_G_down,1)/2),size(x_h_G_down,2));
for i=1:size(x_h_H_down,2)
...
x_h_H_up = zeros(size(x_v_H,1),2*size(x_v_H,2));
x_h_G_up = zeros(size(x_v_G,1),2*size(x_v_G,2));
for i=1:size(x_h_H_up,1)
    x_h_H_up(i,:) = upsample(x_v_H(i,:),2);
    x_h_G_up(i,:) = upsample(x_v_G(i,:),2);
end
